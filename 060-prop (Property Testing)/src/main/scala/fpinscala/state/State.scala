...
object RNG { //You could name it companionObjectRNG
  ...
  // We map over that to prepend "add (something) to the beginning of something else." (cons) the element 
  //onto the accumulated list.
  //
  ...
  def _ints(count: Int): Rand[List[Int]] =
    sequence(List.fill(count)(int))   // val int: Rand[Int] = _.nextInt
...
//The idiomatic (typical of the natural way in which someone speaks or writes when they are using their own 
//language) solution is expressed via foldRight
  ...
  // technically has to also walk the list twice, since it has to unravel (از هم باز کردن) the call
