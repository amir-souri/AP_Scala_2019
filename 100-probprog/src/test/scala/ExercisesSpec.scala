...
class ExercisesSpec extends FreeSpec with Matchers with PropertyChecks {
  ...
  "Exercise 5" - {
    ...
    s"blackBallsNo is uniform in range [0,${Exercises.UpperBound})" in {
      ...
      //val T = List(0 to 10 by 2)   //nashod    List(Range 0 to 10 by 2)

      //val T = (0 to 10 by 2).toList  // shayad beshe   List(0, 2, 4, 6, 8, 10)

      val imp = Importance (5000, Exercises.blackBallsNo)

      // We are using the imperative API of Figaro to speed up tests for students
