...
import com.cra.figaro.language.{Constant, Element, Flip, Universe}
...
object Exercises {
  ...
  // The number of balls, including exactly 1 red ball, found in the urn at the
  ...
  // the red ball.
  // We model the outcome of picking as true, iff the red ball has
  // been picked, and false otherwise. Use the following constructor 'Flip' to
  // implement 'pick'.
  //
  // Flip (probability: Double): Element[Boolean]

      /*Coin flipping, coin tossing, or heads or tails is the practice of throwing a coin in the air and checking which
      side is showing when it lands
      Flip(0.7) is an Element[Boolean] that represents the probabilistic model that produces true with probability 0.7
       and false with probability 0.3.*/
  def pick (n/*# of black balls*/: Int): Element[Boolean] = {
  // val a: Element[Boolean] = Flip(1 / n + 1)
    Flip(1.0 / (n + 1.0))
  }
// Exercise 2.
//
...
// '1'.
  ...
  def move (player: Player, n/*number of black balls*/: Int): Element[Player] = {
    //val te = pick(n)
    //import com.cra.figaro.library.compound.If
    //If(pick(n), Constant(player), move(next(player), n-1))
    pick(n).flatMap((b: Boolean) => if (b) Constant(player) else move(next(player), n - 1))
  }
// Exercise 3.
//
// Peter is polite and offers a choice to Paula(girl), if she wants to start or
// rather had (the same as Would rather) that he started.
//
...
/* BallsNo - 1 since BallsNo is the number of balls, including exactly 1 red ball, but move takes n as the number of
   black balls*/
// Which strategy is better for Paula? What if BallsNo == 9?
//
// Write your answer here in a comment: It is better he starts the game because of parity.
// If BallsNo is odd the it the probability that the first player wins will be slightly increased otherwise will be the same.
...
// it was initial equally likely that Peter and Paula are starting.
//
...
// We first create a uniform prior for the first mover:
  ...
  /*scala> val un = Uniform("a" , "b")
  un: com.cra.figaro.library.atomic.discrete.AtomicUniform[String] = Uniform(a, b)
  scala> Importance.probability(un, "b")
  res1: Double = 0.4912000000000188
  scala> VariableElimination.probability(un, "a")
  res2: Double = 0.5
  */


// Now create a nullary function (A function that takes no arguments. Also known as nullary function.) 'gameResult'
  // that picks the first mover randomly using 'firstMover' and then returns the probability distribution
// for a game played with BallsNo balls in the urn:

 /* Nullary functions are defined as an object (c++ class/struct) that is sometimes otherwise known as callable
 without any argument passing.*/

  def gameResult: Element[Player] = {
   //Importance.probability()
   //val alg = VariableElimination(firstMover)
    //alg.start
//   val test: Element[Element[Player]]  =  firstMover.map(p => move(p, BallsNo - 1))
    firstMover.flatMap(p => move(p, BallsNo - 1)) // -1 since reed is inclusive in ballsNo but move takes n as the #black balls
...
// are not getting used to it :)
  ...
  /*Condition the element by observing a particular value. Propagates the effect to dependent elements and ensures
  that no other value for the element can be generated.*/

  lazy val gameWonByPaula = gameResult
  gameWonByPaula.observe (Paula)

/* observe : Condition the element by observing a particular value. Propagates the effect to dependent elements and
   ensures that no other value for the element can be generated.
  */
...
// Compute the probability that Paula has started
  //Question
  //I cannot see how probPaulaStarted is effected by gameWonByPaula.observe (Paula)! Maybe it has some thing to do with
  //Universe :     Importance.probability(firstMover, Paula) (Universe.universe)?
  //We are supposed to calculate p( PaulaStarted | PaulaWon)
  //Answer: Yes you are right. When you evaluate  gameWonByPaula.observe (Paula) the universe will be the universe that
  //Paula won the game
  val dU = Universe.universe
  /*Most of the time while using Figaro, you will not need to
  create a new universe and can rely on the default universe, which is
    just called universe.*/
def probPaulaStarted: Double = Importance.probability(firstMover, Paula) //(dU)
  //val nU = Universe.createNew()

  /*
  with gameWonByPaula.observe (Paula) :
  scala> Importance.probability(gameWonByPaula, Paula)
  res4: Double = 1.0

  scala> Importance.probability(gameWonByPaula, Peter)
  res5: Double = 0.0

 without gameWonByPaula.observe (Paula):  //recall   val BallsNo: Int = 8 it is even then does not matter who starts
  scala> Importance.probability(gameWonByPaula, Paula)
  res0: Double = 0.5047000000000212

  scala> Importance.probability(gameWonByPaula, Peter)
  res1: Double = 0.4988000000000183

But
   with gameWonByPaula.observe (Paula):
   scala> Importance.probability(firstMover, Paula)
   res4: Double = 0.5714000000000142

   scala> Importance.probability(firstMover, Peter)
   res5: Double = 0.43180000000001306
It is a sign that gameWonByPaula.observe (Paula) has effected on probPaulaStarted
   */
...
// The argument list can be generated using List.tabulate[A].
 //val tab: List[Int] =  List.tabulate(6)(_ + 0) //List(0, 1, 2, 3, 4, 5)
//Question what is the reason for using List.tabulate. Answer: assume that you want a long uniform with a specific property.
lazy val blackBallsNo: Element[Int] = Uniform[Int] (List.tabulate(UpperBound)(_ + 0): _*)  //Uniform[Int]( 0, 2, 6, 4, 8)
//Uniform ((0 to (UpperBound - 1)).toList: _*)  // List.tabulate(UpperBound)(identity)
...
// (the game is entirely symmetric: Uno is not a symmetric game since there might be a situation that a player be skipet
  //or the order wont be in turn.)
// There is no test for this step of the computation.

//def outcome: Element[Player] = blackBallsNo.flatMap(bn => move(Paula, bn))
...
// outcome observe (Paula)  it must change to
//   val o1 = outcome
   outcome observe (Peter)
...
// values generated with 'distribution'.
  // ( distribution: recall the dart example.)

  // Andrzej posteriorOdd function:
//  val posteriorOdd: Double =
//    Importance.probability[Int] (blackBallsNo, (n: Int) => (n+1) % 2 == 1)

  // Mine:
lazy val posteriorOdd: Double =
//  Importance.probability(blackBallsNo, (nu: Int) => (nu % 2 != 0) ) //It is not for some reason ):
  Importance.probability(blackBallsNo, (nu: Int) => ( (nu + 1) % 2 == 1) )

// Is the posteriorOdd greater than 1/2?
//  Yes. // (because with the odd number of moves, the first player has one more move that gives a chance to win).
//  Paula started Paula won: probability of urn has odd number
//  scala> posteriorOdd
//  res0: Double = 0.5963000000000114

//  Peter started Peter won: probability of urn has odd number
// scala> posteriorOdd
//  res1: Double = 0.6070000000000118


//  Peter started Paula won: probability of urn has odd number
//  scala> posteriorOdd
//  res0: Double = 0.3242000000000054

//  Paula started Peter won: probability of urn has odd number
//  scala> posteriorOdd
//  res0: Double = 0.3397000000000059
