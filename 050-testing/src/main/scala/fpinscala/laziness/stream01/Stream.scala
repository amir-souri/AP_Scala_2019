...
trait Stream[+A] {
  ...
  */
  ...
  //implement take so that it force the head and tail just for seeing if my test fails
  def take(n: Int): List[A] = this match {
    case Cons(h, t) if n > 1 => this.toList
    case Cons(h, _) if n == 1 => this.toList
    case _ => this.toList
  }
