...
import Matchers._
  ...
  class LensesSpec extends FlatSpec with Checkers {
    ...
    /// when you pass a lens to PutGet the type parameters C and A will be defined as the passed lens
...
//      forAll((n: Int) => n + 0 == n)
      ...
      ///lens.get(lens.set(a)(s)) <==> a in Monocle
    ...
    def GetPut[C,A] (l: Lens[C, A])(implicit aA: Arbitrary[A], aC: Arbitrary[C]) :Prop = {
      ...
      ///lens.set(lens.get(s))(s) <==> s   in Monocle
    }
    ...
    ///set (s, set (s' , (c, n))) = (s, n + 2) != (s, n + 1) = set (s, (c, n)).
     ...
     def PutPut[C,A] (l: Lens[C, A])(implicit aA: Arbitrary[A], aC: Arbitrary[C]) :Prop = {
       ...
       ///lens.set(a)(lens.set(a)(s)) <==> lens.set(a)(s) in Monocle
    }


//    "PutPut law for l3 : set(\"Amir\")((set(\"Andrzej\")((\"Zhoulai\", 10))))" should "should be (\"Amir\", 11)" in  {
//      assert(l3.set("Amir")((l3.set("Andrzej")(("Zhoulai", 10)))) == l3.set("Amir")(("Zhoulai", 10)))
//    }
...
//    "Amir-id1" should behave like veryWellBehavedTotalLense(id1)

//////////////////////////////////////Wright test for exam//////////////////////////////////////////////////////////
//    "exam2017-nullOption" should behave like veryWellBehavedTotalLense(nullOption[Int])

//    "exam2018" should behave like veryWellBehavedTotalLense (eitherOption[Int,Int](4))

//    "exam2019" should behave like veryWellBehavedTotalLense ()


    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //     "l1" should behave like wellBehavedTotalLense (l1) // will fail GetPut see p. 6   (set what you get)
//     "l2" should behave like wellBehavedTotalLense (l2) // will fail PutGet see p. 6   (get what you set)
//     "l3" should behave like wellBehavedTotalLense (l3)
//     "l3" should behave like veryWellBehavedTotalLense (l3) /// will fail PutPut see p. 7 (but for a corner case)
    /// "l3" will show
    /// l3
    ///[info] - should obey the PutGet law
    ///[info] - should obey the GetPut law
    ///[info] l3
    ///[info] - should obey the PutPut law *** FAILED ***
    /// but it will show
    /// l3
    ///[info] - should obey the PutGet law
    ///[info] - should obey the GetPut law
    ...
    // The four codiag lines unfortunately do not work with scalacheck 0.14
    // scalatest 3.0.5 and monocle 1.5 + scala 2.12.4.  It appears that this is
...
//     { assert (itu3.students.values.map(_.country).forall (s => s.toUpperCase == s)) }
    ...
    //optional.getOption(optional.set(a)(s)) <==> optional.getOption(s).map(_ => a)
      ...
      //forAll{ (a: A, c: C) =>   l.getOption( l.set (a) (c) ) == Some(a) }
...
//       forAll { (a: A, c: C) => l.set (  l.getOption(c) )  (c) == c } // you can get a value from Some by .get
      forAll((a: A, c: C) => {
        l.set(l.getOption(c).get) (c) == c
      }
      )
...
// that we can quickly test wheter our implemented lenses fulfil the laws using
// pre-implemented tests.
//
// They do not implement PutPut though.
// Find the laws for Lenses here:
//https://github.com/julien-truffaut/Monocle/blob/master/core/shared/src/main/scala/monocle/law/LensLaws.scala
//
// Find the laws for Optionals here:
//https://github.com/julien-truffaut/Monocle/blob/master/core/shared/src/main/scala/monocle/law/OptionalLaws.scala
